...
<div class="container">
    ...
    <div class="page-header">
        <h1><i class="fas fa-book-open me-2"></i> Daftar Mata Pelajaran</h1>
    </div>

    <!-- Filter Panel -->
    <div class="filter-panel">
        <form method="GET" action="{{ route('mata-pelajaran.index') }}" class="row g-3">
            <div class="col-md-4">
                <div class="input-group">
                    <span class="input-group-text bg-white"><i class="fas fa-search"></i></span>
                    <input type="text" name="search" class="form-control" placeholder="Cari kode atau mapel..." value="{{ request('search') }}">
                </div>
            </div>
            ...
            <div class="col-md-1">
                <a href="{{ route('mata-pelajaran.index') }}" class="btn btn-outline-danger w-100">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="d-inline-block">
            <span class="badge bg-secondary px-3 py-2">Total: {{ $mapel->total() }} Mapel</span>
        </div>
    </div>
                ...
                <tbody>
                    @forelse($mapel as $index => $item)
                        <tr>
                            ...
                            <td>
                                <span class="text-primary">{{ $item->mata_pelajaran }}</span>
                            </td>
                            <td>
                                <div class="btn-action-container">
                                    <a href="{{ route('mata-pelajaran.edit', $item->id) }}" class="btn btn-warning btn-action">
                                        <i class="fas fa-edit me-1"></i> Edit
                                    </a>
                                    <form action="{{ route('mata-pelajaran.destroy', $item->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-action" onclick="return confirm('Yakin ingin menghapus mapel ini?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-muted">
                                <i class="fas fa-info-circle me-2"></i>Tidak ada data mata pelajaran.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
